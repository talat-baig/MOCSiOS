...
class FundsRemittancListVC: UIViewController , filterViewDelegate, clearFilterDelegate, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        Helper.setupTableView(tableVw: self.tableView, nibName: "FRemListCell" )
        self.refreshList()
    }
    ...
    @objc func populateList() {
        ...
        var newData :[FRListData] = []
        
        if internetStatus != .notReachable {
            
            let url = String.init(format: Constant.FundsRemittance.FR_LIST, Session.authKey,
                ...
                if Helper.isResponseValid(vc: self, response: response.result,tv: self.tableView){
                    ...
                    if arrayJson.count > 0 {
                        ...
                        do {
                            ...
                            // 2
                            newData = try decoder.decode([FRListData].self, from: response.result.value!)
                        } catch let error { // 3
...
extension FundsRemittancListVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! FRemListCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let fpInstrmntVC = self.storyboard?.instantiateViewController(withIdentifier: "InstrumentListVC") as! InstrumentListVC
        fpInstrmntVC.refID = self.arrayList[indexPath.row].refID ?? ""
        self.navigationController?.pushViewController(fpInstrmntVC, animated: true)
    }
    
}

extension FundsRemittancListVC: UISearchBarDelegate {
...
extension FundsRemittancListVC: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
