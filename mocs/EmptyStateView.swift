...
import Foundation
import UIKit
class EmptyStateView:UIControl{
    /// The image view
    fileprivate(set) open var upperView: UIView!
...
//        }
    ...
    ///   - controlEvents: A bitmask specifying the control-specific events for which the action method is called. Always specify at least one constant. For a list of possible constants, see UIControl.Event.
...
//        button.addTarget(target, action: action, for: controlEvents)
    ...
    open func setupStack(_ view : UIView ,_ imageView: UIImageView,_ textLabel: UILabel)->UIStackView{
        let stackView = UIStackView(arrangedSubviews: [view,imageView,textLabel])
...
//        button = setupButton()
        let stackView = setupStack(upperView, imageView,textLabel)
        ...
        if (UIDevice.current.userInterfaceIdiom == .pad) {
            ...
            upperView.heightAnchor.constraint(equalToConstant: 90).isActive = true
